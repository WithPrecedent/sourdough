...
class Structure(sourdough.OptionsMixin, sourdough.Component):
    ...
    iterators: ClassVar['sourdough.Catalog'] = sourdough.Catalog(
        contents = {
            ...
            'default': iter,
            'flat': more_itertools.collapse,
            'permutations': itertools.permutations,
            'selected': itertools.compress,
            'study': itertools.product}) 
    options: ClassVar['sourdough.Catalog'] = sourdough.Catalog(
        contents = {
            'builder': sourdough.Builder,
            'edge': sourdough.Edge,
            'node': sourdough.Node,
    ...
    def iterator(self) -> NotImplementedError:
        """Raises NotImplementedError unless overwritten by subclass.
        
        Raises:
            NotImplementedError: there is no custom iterator in the base class,
                but subclasses can implement selectable iterators by overwriting
                this method.
                
        """
        raise NotImplementedError(
            f'A custom iterator is not available in {self.name}')
            
    def set_iterator(self, 
            iterator: Union[str, Callable],
        ...
        """
        if isinstance(iterator, Callable):
            component.__iter__ = iterator(component.contents)
        elif isinstance(self.iterators[iterator], str):
            component.__iter__ = getattr(self.iterators[iterator])(
                component.contents)
        elif isinstance(self.iterators[iterator], Callable):
            component.__iter__ = self.iterators[iterator](component.contents)
        else:
            raise TypeError('iterator must be a str or Callable')    
        return component
...
class Chain(Structure):
    ...
    name: str = None
    iterator: Union[str, Callable] = 'chain'  
    options: ClassVar['sourdough.Catalog'] = sourdough.Catalog(
        contents = {
            'builder': sourdough.Builder,
            'technique': sourdough.Technique,
...
class Tree(Structure):
    ...
    iterators: ClassVar['sourdough.Catalog'] = sourdough.Catalog(
        contents = {
            'compare': itertools.product,
            'custom': 'iterator',
            'flattened': more_itertools.collapse,
            }) 
    options: ClassVar['sourdough.Catalog'] = sourdough.Catalog(
        contents = {
            'builder': sourdough.Builder,
...
class Graph(Structure):
    ...
    @abc.abstractmethod
    def validate(self, 
            graph: 'sourdough.Worker' = None) -> None:
        """Subclasses must provide their own methods."""
